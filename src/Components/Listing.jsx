...
const PROPERTIES = [
  {
    ...
    baths: 2,
    featured: true,
    listedAt: new Date("2025-02-05").getTime(),
    images: [
  ...
  },
  {
    ...
    baths: 4,
    featured: true,
    listedAt: new Date("2025-01-25").getTime(),
    images: [
      "https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=1600&q=80",
      "https://images.unsplash.com/photo-1505691723518-36a5ac3b2d95?auto=format&fit=crop&w=1600&q=80",
    ],
  },
  {
    id: "RPR-1003",
    title: "Residential Plot in Atal Nagar",
    type: "Land",
    city: "Naya Raipur (Atal Nagar)",
    location: "Sector 19, Atal Nagar",
    price: 2200000,
    area: 1200,
    beds: null,
    baths: null,
    featured: false,
    listedAt: new Date("2024-12-28").getTime(),
    images: [
  ...
  },
  {
    ...
    baths: 2,
    featured: false,
    listedAt: new Date("2025-02-01").getTime(),
    images: [
  ...
  },
  {
    ...
    baths: 5,
    featured: true,
    listedAt: new Date("2025-01-12").getTime(),
    images: [
  ...
  {
    id: "RPR-1006",
    title: "Plot in Kamal Vihar Sector 4",
    type: "Land",
    city: "Kamal Vihar",
    location: "Kamal Vihar, Sector 4, Raipur",
    price: 3000000,
    area: 1500,
    beds: null,
    baths: null,
    featured: false,
    listedAt: new Date("2025-01-08").getTime(),
    images: [
  ...
  },
  {
    ...
    baths: 3,
    featured: false,
    listedAt: new Date("2025-02-07").getTime(),
    images: [
  ...
  },
  {
    ...
    baths: 2,
    featured: false,
    listedAt: new Date("2025-01-30").getTime(),
    images: [
  ...
  {
    id: "RPR-1009",
    title: "Open Plot on Vidhan Sabha Road",
    type: "Land",
    city: "Saddu",
    ...
    baths: null,
    featured: false,
    listedAt: new Date("2024-12-22").getTime(),
    images: [
  ...
  {
    id: "RPR-1010",
    title: "2BHK in Mowa (Ring Road 1)",
    type: "Apartment",
    city: "Mowa",
    ...
    baths: 2,
    featured: false,
    listedAt: new Date("2025-02-10").getTime(),
    images: [
...
// Note: 'city' field here actually stores Raipur localities
...
function Badge({ children, color = "emerald" }) {
  // Keeping badge colors same (only buttons/focus are orange as per your preference)
  const colors = {
...
export default function PropertySearch() {
  ...
  const [city, setCity] = useState("");       // city here = locality within Raipur
  ...
  const filtered = useMemo(() => {
    const [pMin, pMax] = clampRange(
      minPrice === "" ? NaN : Number(minPrice),
      maxPrice === "" ? NaN : Number(maxPrice)
    );
    const [aMin, aMax] = clampRange(
      minArea === "" ? NaN : Number(minArea),
      maxArea === "" ? NaN : Number(maxArea)
    );

    let arr = PROPERTIES.filter((p) => {
      const matchesQuery =
        !q || includesCI(p.title, q) || includesCI(p.location, q) || includesCI(p.city, q);

      const matchesCity = !city || p.city === city;      // locality match
      const matchesType = !type || p.type === type;

      const matchesPrice =
        (pMin === -Infinity || p.price >= pMin) && (pMax === Infinity || p.price <= pMax);

      const matchesArea =
        (aMin === -Infinity || p.area >= aMin) && (aMax === Infinity || p.area <= aMax);

      return matchesQuery && matchesCity && matchesType && matchesPrice && matchesArea;
    });
    ...
    return arr;
  }, [q, city, type, minPrice, maxPrice, minArea, maxArea, sortBy]);
  ...
  return (
    <section className="relative w-full bg-slate-50 py-8 sm:py-10 lg:py-12">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-slate-600">
              Filter by price, locality, size and type. Fully responsive and fast.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
            >
        ...
        {/* Filters */}
        <div className="mt-4 rounded-2xl bg-white/90 backdrop-blur ring-1 ring-slate-200 shadow-sm p-4 sm:p-5 lg:p-6">
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            ...
            <div className="lg:col-span-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Keyword, locality (Raipur)"
                  className="w-full rounded-lg border border-slate-200 bg-white pl-10 pr-3 py-2.5 text-slate-800 placeholder-slate-400 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
                />
              </div>
            </div>

            {/* Locality (using 'city' field) */}
            <div>
              <label className="sr-only" htmlFor="city">Locality (Raipur)</label>
              <select
                id="city"
                value={city}
                onChange={(e) => setCity(e.target.value)}
                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-slate-800 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
              >
                <option value="">All localities</option>
                {uniqueCities.filter((c) => c).map((c) => (
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-slate-800 placeholder-slate-400 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100"
              />
            </div>
